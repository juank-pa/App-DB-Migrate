...
use App::DB::Migrate::Constraint::PrimaryKey;

subtest 'new creates a primary key' => sub {
    my $pk = App::DB::Migrate::Constraint::PrimaryKey->new('table', 'column');
    isa_ok($pk, 'App::DB::Migrate::Constraint::PrimaryKey');
    isa_ok($pk, 'App::DB::Migrate::Constraint');
};

subtest 'new is invalid if table is not sent' => sub {
    trap { App::DB::Migrate::Constraint::PrimaryKey->new() };
    like($trap->die, qr/^Table name needed/);
};

subtest 'new is invalid if table is not sent' => sub {
    trap { App::DB::Migrate::Constraint::PrimaryKey->new('users') };
    like($trap->die, qr/^Column name needed/);
};

subtest 'is SQLizable' => sub {
    my $pk = App::DB::Migrate::Constraint::PrimaryKey->new('users', 'name');
    isa_ok($pk, "App::DB::Migrate::SQLizable");
};

subtest 'autoincrements returns the autoincrement option' => sub {
    my $pk = App::DB::Migrate::Constraint::PrimaryKey->new('users', 'column', { autoincrement => 1 });
    ok($pk->autoincrements);

    $pk = App::DB::Migrate::Constraint::PrimaryKey->new('users', 'column');
    ok(!$pk->autoincrements);
};

subtest 'table returns the table name' => sub {
    my $pk = App::DB::Migrate::Constraint::PrimaryKey->new('users', 'column');
    is($pk->table, 'users');
};
...
subtest 'to_sql returns SQL representation of primary key with autoincrement' => sub {
    my $pk = App::DB::Migrate::Constraint::PrimaryKey->new('users', 'id', { autoincrement => 1 });
