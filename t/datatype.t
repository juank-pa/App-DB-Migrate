...
use App::DB::Migrate::Datatype;

subtest 'new default implementation is always invalid' => sub {
    trap { App::DB::Migrate::Datatype->new('integer') };
    like($trap->die, qr/^Invalid datatype: integer/);
};

my $module = new Test::MockModule('App::DB::Migrate::Datatype');
...
subtest 'new returns a valid object if the name is valid' => sub {
    my $dt = App::DB::Migrate::Datatype->new('integer');
    is($dt->name, 'integer');
};

subtest 'new is invalid if not a valid datatype' => sub {
    trap { App::DB::Migrate::Datatype->new('not_valid_dt') };
...
subtest 'new is valid and defaults to string if datatype looks like number' => sub {
    my $dt = App::DB::Migrate::Datatype->new(1);
    is($dt->name, 'string');

    $dt = App::DB::Migrate::Datatype->new(145);
    is($dt->name, 'string');
};

subtest 'new fallsback to a default datatype if not specified' => sub {
    my $dt = App::DB::Migrate::Datatype->new();
    is($dt->name, 'string');
};

subtest 'is SQLizable' => sub {
    my $col = App::DB::Migrate::Datatype->new('integer');
    isa_ok($col, "App::DB::Migrate::SQLizable");
};

subtest 'name returns the datatype API name' => sub {
    my $dt = App::DB::Migrate::Datatype->new('string');
    is($dt->name, 'string');
};
...
subtest 'limit returns the limit from initialization' => sub {
    my $dt = App::DB::Migrate::Datatype->new('string', { limit => 34 });
    is($dt->limit, 34);
};

subtest 'precision returns the precision from initialization' => sub {
    my $dt = App::DB::Migrate::Datatype->new('string', { precision => 12 });
    is($dt->precision, 12);
};

subtest 'scale returns the scale from initialization' => sub {
    my $dt = App::DB::Migrate::Datatype->new('string', { scale => 52 });
    is($dt->scale, 52);
};
...
subtest 'imports get_dbh from App::DB::Migrate::Dbh' => sub {
    ...
    is(\&App::DB::Migrate::Dbh::get_dbh, \&App::DB::Migrate::Datatype::get_dbh);
};

subtest 'quote_str quotes the given value unconditionally' => sub {
    my $module = new Test::MockModule('App::DB::Migrate::Datatype');
    $module->redefine('get_dbh', get_dbh);
    is(App::DB::Migrate::Datatype->quote_str("I'm quoted"), "/I'm quoted/");
};
...
subtest 'quote does not quote the given value depending on the datatype' => sub {
    my $dt = App::DB::Migrate::Datatype->new('integer');
    my $module = new Test::MockModule('App::DB::Migrate::Datatype')->mock('get_dbh', get_dbh);
    is($dt->quote(45), 45);
};
...
subtest 'to_sql returns the DB datatype with a limit if limit is set' => sub {
    my $dt = App::DB::Migrate::Datatype->new('string', { limit => 33 });
    is($dt->to_sql, 'VARCHAR(33)');
};

subtest 'to_sql returns the DB datatype with a precision if precision is set' => sub {
    my $dt = App::DB::Migrate::Datatype->new('integer', { precision => 83 });
