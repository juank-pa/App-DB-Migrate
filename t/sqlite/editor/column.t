...
subtest 'new fails if column name is undef' => sub {
    trap { Migrate::SQLite::Editor::Column->new() };
    like($trap->die, qr/^Column name needed/);
};

subtest 'is SQLizable' => sub {
    my $cns = Migrate::SQLite::Editor::Column->new('my_name', 'null');
    isa_ok($cns, 'Migrate::SQLizable');
};

subtest 'name returns the column name' => sub {
    my $col = Migrate::SQLite::Editor::Column->new('col1');
    is($col->name, 'col1');
};
...
subtest 'rename fails is new name is undef' => sub {
    trap { Migrate::SQLite::Editor::Column->new('name')->rename() };
    like($trap->die, qr/^Column name needed/);
};

subtest 'type returns the column type' => sub {
    my $dt = Migrate::SQLite::Editor::Datatype->new('INT');
    my $col = Migrate::SQLite::Editor::Column->new('name', $dt);
    is($col->type, $dt);
    is($col->type->native_name, 'INT');
};

subtest 'type returns an undefined datatype is none is given' => sub {
    my $col = Migrate::SQLite::Editor::Column->new('name');
    is($col->type->native_name, '');
};

subtest 'constraints returns the list of constraints' => sub {
    my $c1 = Migrate::SQLite::Editor::Constraint->new(undef, 'NOT NULL');
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $c1);
    is($col->constraints->[0], 'ANY');
    is($col->constraints->[1], $c1);
};

subtest 'is_null returns true if there is not any NOT NULL constraint' => sub {
    my $df = Migrate::SQLite::Editor::Constraint->new(undef, 'DEFAULT', 5);
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $df);
    ok($col->is_null);
};

subtest 'is_null returns false if there is a NOT NULL constraint' => sub {
    my $nl = Migrate::SQLite::Editor::Constraint->new(undef, 'not null');
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $nl);
    ok(!$col->is_null);
};
...
subtest 'change_null does nothing if status is equal to parameter' => sub {
    ...
    $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', 'THING');
...
subtest 'change_null removes a not null if true is sent' => sub {
    my $nl = Migrate::SQLite::Editor::Constraint->new(undef, 'not null');
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $nl, 'THING');
...
subtest 'change_null adds a not null constraint if false is sent' => sub {
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', 'THING');
    my $res = $col->change_null(0);
    ok($res);
    is(scalar @{ $col->constraints }, 3);
    is_deeply($col->constraints, ['ANY', 'THING', 'NOT NULL']);
    isa_ok($col->constraints->[2], 'Migrate::SQLite::Editor::Constraint');
...
subtest 'change_null removes any previous null and adds a not null constraint if false is sent' => sub {
    my $nl = Migrate::SQLite::Editor::Constraint->new(undef, 'null');
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $nl, 'THING');
    my $res = $col->change_null(0);
    ok($res);
    is(scalar @{ $col->constraints }, 3);
...
subtest 'default_constraint returns the default constraint if it exists' => sub {
    my $df = Migrate::SQLite::Editor::Constraint->new(undef, 'default', 5);
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $df, 'THING');
    isa_ok($col->default_constraint, 'Migrate::SQLite::Editor::Constraint');
...
subtest 'change_default updates a default (value only) if it already exists' => sub {
    my $df = Migrate::SQLite::Editor::Constraint->new('any_name', 'default', "'val'");
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $df, 'THING');
    ...
    isa_ok($col->constraints->[1], 'Migrate::SQLite::Editor::Constraint');
...
subtest 'change_default does not quote new value depending on dataype' => sub {
    my $df = Migrate::SQLite::Editor::Constraint->new('any_name', 'default', "'val'");
    my $dt = Migrate::SQLite::Editor::Datatype->new('INT');
    my $col = Migrate::SQLite::Editor::Column->new('name', $dt, 'ANY', $df, 'THING');
...
subtest 'foreign_key_constraint returns the foreign key constrain if it exists' => sub {
    my $fk = Migrate::SQLite::Editor::Constraint->new(undef, 'REFERENCES', 'table');
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $fk, 'THING');
    isa_ok($col->foreign_key_constraint, 'Migrate::SQLite::Editor::Constraint');
...
subtest 'add_foreign_key does nothing if a foreign key exists' => sub {
    my $fk = Migrate::SQLite::Editor::Constraint->new('any', 'REFERENCES', 'table');
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $fk, 'THING');
...
subtest 'add_foreign_key can receive anything that serializes as a constraint' => sub {
    ...
    $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', 'THING');
    $fk = Migrate::Constraint::ForeignKey->new('users', 'departments');
...
subtest 'add_foreign_key fails if string cannot be parsed as a foreign_key' => sub {
    ...
    $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', 'THING');
    my $dt = Migrate::SQLite::Datatype->new('integer');
    my $fk = Migrate::Constraint::Default->new(5, { type => $dt });
...
subtest 'has_constraint_named returns true if a constraint with a given name exists' => sub {
    my $fk = Migrate::SQLite::Editor::Constraint->new('anything', 'REFERENCES', 'table');
    my $col = Migrate::SQLite::Editor::Column->new('name', undef, 'ANY', $fk, 'THING');
...
subtest 'to_sql returns a SQL representation of the column without datatype' => sub {
    my $col = Migrate::SQLite::Editor::Column->new('col"name', undef);
    is($col->to_sql, '"col""name"');
};

subtest 'to_sql returns a SQL representation of the column with datatype' => sub {
    my $col = Migrate::SQLite::Editor::Column->new('col"name', 'DOUBLE');
    is($col->to_sql, '"col""name" DOUBLE');
};

subtest 'to_sql returns a SQL representation of the column with constraints' => sub {
    my $fk = Migrate::SQLite::Editor::Constraint->new('any', 'references', 'table', '(c1)');
    my $col = Migrate::SQLite::Editor::Column->new('col_name', 'VARCHAR', 'ANY', $fk, 'thing');
