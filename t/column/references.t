...
subtest 'new overrides the column name' => sub {
    App::DB::Migrate::Column::References->new('table', 'column', { foreign_key => { column => 'new_col_name' } });
    ok_factory_nth(COL, 0, 'new_col_name');
};

subtest 'new passes integer as the column datatype' => sub {
    App::DB::Migrate::Column::References->new('table', 'column');
    ok_factory_nth(COL, 1, 'integer');
};

subtest 'new overrides the column datatype' => sub {
    App::DB::Migrate::Column::References->new('table', 'column', { type => 'anytype' });
    ok_factory_nth(COL, 1, 'anytype');
};

subtest 'new passes options to column' => sub {
    App::DB::Migrate::Column::References->new('table', 'my_ref', { any => 'options' });
...
subtest 'new can create a foreign key from table' => sub {
    App::DB::Migrate::Column::References->new('my_table', 'my_ref', { foreign_key => 1 });
    ok_factory_nth(FK, 0, 'my_table');
};

subtest 'new can create a foreign key to pruralized ref_name table' => sub {
    App::DB::Migrate::Column::References->new('table', 'departments', { foreign_key => 1 });
    ok_factory_nth(FK, 1, 'departments');
};

subtest 'new can create a foreign key with overridden to table' => sub {
    App::DB::Migrate::Column::References->new('table', 'departments', { foreign_key => { to_table => 'new_table' } });
    ok_factory_nth(FK, 1, 'new_table');
};

subtest 'new passes options to foreign key' => sub {
    App::DB::Migrate::Column::References->new('table', 'departments', { foreign_key => { any => 'options' } });
...
subtest 'delegates methods to column' => sub {
    ...
    my $ref = App::DB::Migrate::Column::References->new('my_table', 'col');
...
subtest 'is SQLizable' => sub {
    my $ref = App::DB::Migrate::Column::References->new('table', 'col');
    isa_ok($ref, "App::DB::Migrate::SQLizable");
};

subtest 'foreign_key_constraint returns the foreign key', => sub {
    my $ref = App::DB::Migrate::Column::References->new('table', 'col', { foreign_key => 1 });
...
subtest 'table returns the table name', => sub {
    my $ref = App::DB::Migrate::Column::References->new('table_name', 'col');
    is($ref->table, 'table_name');
};

subtest 'raw_name returns the non-processed ref_name', => sub {
    my $ref = App::DB::Migrate::Column::References->new('table_name', 'pre_col');
