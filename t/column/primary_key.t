...
subtest 'new sets the column name to a custom name if provided' => sub {
    App::DB::Migrate::Column::PrimaryKey->new('my_table', 'my_id_custom');
...
subtest 'new sets the column dataype to a custom type if provided' => sub {
    App::DB::Migrate::Column::PrimaryKey->new('my_table', { type => 'whatever_type' });
    ok_factory_nth(COL, 1, 'whatever_type');

    App::DB::Migrate::Column::PrimaryKey->new('my_table', 'col', { type => 'whatever_type' });
    ok_factory_nth(COL, 1, 'whatever_type');
};

subtest 'new passes options to column' => sub {
    App::DB::Migrate::Column::PrimaryKey->new('my_table', { any => 'option' });
    ok_factory_nth(COL, 2, { any => 'option' });

    App::DB::Migrate::Column::PrimaryKey->new('my_table', 'col', { other => 'option' });
...
subtest 'new passes the table name to the primary key constraint' => sub {
    App::DB::Migrate::Column::PrimaryKey->new('my_table', { autoincrement => 1 });
    ok_factory_nth(PK, 0, 'my_table');
};

subtest 'new passes the column name to the primary key constraint' => sub {
    App::DB::Migrate::Column::PrimaryKey->new('my_table', 'my_column_name', { autoincrement => 1 });
...
subtest 'new does not set primary key constraint to autoincrement when any other type' => sub {
    App::DB::Migrate::Column::PrimaryKey->new('my_table', { type => 'string', autoincrement => 1 });
    ok_factory_nth(PK, 2, { });
};

subtest 'new sets a custom name for the primary key constraint' => sub {
    App::DB::Migrate::Column::PrimaryKey->new('my_table', { name => 'custom_name' });
...
subtest 'delegates methods to column' => sub {
    ...
    my $pk = App::DB::Migrate::Column::PrimaryKey->new('my_table', { name => 'custom_name' });
...
subtest 'is SQLizable' => sub {
    my $pk = App::DB::Migrate::Column::PrimaryKey->new('table');
    isa_ok($pk, "App::DB::Migrate::SQLizable");
};
